...
exports.getEwalets = (req, res) => {
  ...
  db.query("SELECT * FROM ewalet WHERE cafe_id = ? ORDER BY id DESC", [cafeId], (err, results) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal mengambil data ewallet", []);
    }
...
exports.getEwaletById = (req, res) => {
  ...
  db.query("SELECT * FROM ewalet WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, results) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal mengambil detail ewallet", []);
    }
...
exports.createEwalet = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal menambahkan ewallet", []);
      }
...
exports.updateEwalet = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal update ewallet", []);
      }
...
exports.deleteEwalet = (req, res) => {
  ...
  db.query("DELETE FROM ewalet WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, result) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal hapus ewallet", []);
    }
    if (!result || result.affectedRows === 0) {
      return sendResponse(res, 404, "Ewallet tidak ditemukan", []);
    }
