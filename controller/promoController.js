...
exports.getPromos = (req, res) => {
  ...
  db.query("SELECT * FROM promo WHERE cafe_id = ? ORDER BY id DESC", [cafeId], (err, results) => {
    if (err) {
      const pub = toPublicError(err, "Gagal mengambil data promo");
...
exports.getPromoById = (req, res) => {
  ...
  db.query("SELECT * FROM promo WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, results) => {
    if (err) {
      const pub = toPublicError(err, "Gagal mengambil detail promo");
      return sendResponse(res, pub.status, pub.message, []);
    }
...
db.query(
  ...
  [
    ...
    (err, result) => {
      if (err) {
        const pub = toPublicError(err, "Gagal menambahkan promo");
        return sendResponse(res, pub.status, pub.message, []);
      }
...
db.query(
  ...
  [
    ...
    (err, result) => {
      if (err) {
        const pub = toPublicError(err, "Gagal update promo");
        return sendResponse(res, pub.status, pub.message, []);
      }
...
exports.deletePromo = (req, res) => {
  ...
  db.query("DELETE FROM promo WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, result) => {
    if (err) {
      const pub = toPublicError(err, "Gagal hapus promo");
      return sendResponse(res, pub.status, pub.message, []);
    }
    if (!result || result.affectedRows === 0) {
      return sendResponse(res, 404, "Promo tidak ditemukan", []);
...
exports.getPromosPublic = (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        const pub = toPublicError(err, "Gagal mengambil promo");
        return sendResponse(res, pub.status, pub.message, []);
      }
...
exports.validatePromo = (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        const pub = toPublicError(err, "Gagal validasi promo");
