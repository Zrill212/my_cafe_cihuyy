...
exports.getBankTransfers = (req, res) => {
  ...
  db.query("SELECT * FROM bank_transfer WHERE cafe_id = ? ORDER BY id DESC", [cafeId], (err, results) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal mengambil data bank transfer", []);
    }
...
exports.getBankTransferById = (req, res) => {
  ...
  db.query("SELECT * FROM bank_transfer WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, results) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal mengambil detail bank transfer", []);
    }
...
exports.createBankTransfer = (req, res) => {
  ...
  if (accounts && accounts.length > 0) {
    ...
    db.query(
      ...
      (err) => {
        if (err) {
          return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal menambahkan bank transfer", []);
        }
...
exports.updateBankTransfer = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal update bank transfer", []);
      }
...
exports.deleteBankTransfer = (req, res) => {
  ...
  db.query("DELETE FROM bank_transfer WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, result) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal hapus bank transfer", []);
    }
    if (!result || result.affectedRows === 0) {
      return sendResponse(res, 404, "Bank transfer tidak ditemukan", []);
    }
