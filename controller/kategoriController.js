...
exports.getKategoris = (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        const pub = toPublicError(err, "Gagal mengambil data kategori");
...
exports.getKategoriById = (req, res) => {
  ...
  db.query(
    ...
    (err, results) => {
      if (err) {
        const pub = toPublicError(err, "Gagal mengambil detail kategori");
        return sendResponse(res, pub.status, pub.message, []);
      }
...
exports.createKategori = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        const pub = toPublicError(err, "Gagal menambahkan kategori");
        return sendResponse(res, pub.status, pub.message, []);
      }
      cache.del(cache.buildKey("kategoris", "public", cafeId));
  ...
  db.query(
    ...
    (err, results) => {
      ...
      db.query(query, values, (err2, result) => {
        if (err2) {
          const pub = toPublicError(err2, "Gagal update kategori");
          return sendResponse(res, pub.status, pub.message, []);
        }
  ...
  db.query(
    ...
    (err, results) => {
      ...
      db.query(
        ...
        (err2, result) => {
          if (err2) {
            const pub = toPublicError(err2, "Gagal hapus kategori");
            return sendResponse(res, pub.status, pub.message, []);
          }
          if (!result || result.affectedRows === 0) {
            return sendResponse(res, 404, "Kategori tidak ditemukan", []);
...
exports.getKategorisPublic = (req, res) => {
  ...
  const key = cache.buildKey("kategoris", "public", cafeId);
  const ttl = Number(process.env.CACHE_TTL_KATEGORIS_PUBLIC || 120);

  cache
    .getJSON(key)
    .then((hit) => {
      if (hit) return sendResponse(res, 200, "Berhasil mengambil kategori", hit);
      ...
      db.query(
        ...
        (err, results) => {
          if (err) {
            const pub = toPublicError(err, "Gagal mengambil kategori");
            return sendResponse(res, pub.status, pub.message, []);
          }
          const payload = results || [];
          cache.setJSON(key, payload, ttl);
          return sendResponse(res, 200, "Berhasil mengambil kategori", payload);
        }
      );
    })
    .catch(() => {
