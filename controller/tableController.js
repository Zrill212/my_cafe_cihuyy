...
exports.getTables = (req, res) => {
  ...
  db.query("SELECT * FROM table_cafe WHERE cafe_id = ? ORDER BY nomor_meja ASC", [cafeId], (err, results) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal mengambil data meja", []);
    }
...
exports.getTableQR = (req, res) => {
  ...
  db.query("SELECT qr_code, nomor_meja FROM table_cafe WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, results) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal mengambil QR meja", []);
    }
...
exports.getTableById = (req, res) => {
  ...
  db.query("SELECT * FROM table_cafe WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, results) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal mengambil detail meja", []);
    }
    if (!results || results.length === 0) {
      return sendResponse(res, 404, "Meja tidak ditemukan", []);
    }
...
exports.createTable = async (req, res) => {
  ...
  db.query("SELECT MAX(nomor_meja) AS lastNomor FROM table_cafe WHERE cafe_id = ?", [cafeId], async (err, rows) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal menentukan nomor meja", []);
    }
    ...
    db.query(
      ...
      (err2, result) => {
        if (err2) {
          return sendResponse(res, 500, err2?.sqlMessage || err2?.message || "Gagal menambahkan meja", []);
        }
...
exports.updateTable = (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal update meja", []);
      }
...
exports.deleteTable = (req, res) => {
  ...
  db.query("DELETE FROM table_cafe WHERE id = ? AND cafe_id = ?", [id, cafeId], (err, result) => {
    if (err) {
      return sendResponse(res, 500, err?.sqlMessage || err?.message || "Gagal menghapus meja", []);
    }
    if (!result || result.affectedRows === 0) {
      return sendResponse(res, 404, "Meja tidak ditemukan", []);
    }
